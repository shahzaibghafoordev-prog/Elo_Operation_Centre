                  ...
                  {displayed.length === 0
                    ...
                    : [...displayed].reverse().map((r, i) => (
                      <tr key={i} className={`border-b border-[#1e2a35]/50 hover:bg-[rgba(255,255,255,0.01)] transition-colors ${r.status?.toLowerCase() !== 'complete' ? 'bg-[rgba(255,181,71,0.02)]' : ''}`}>
                        ...
                        <td className="px-4 py-2.5 text-[11px] text-[#a0aec0] max-w-[200px] truncate" title={r.reason || '—'">{r.reason || '—'}</td>
                        <td className="px-4 py-2.5 text-[11px] text-[#718096] max-w-[180px] truncate" title={r.condition || '—'">{r.condition || '—'}</td>
