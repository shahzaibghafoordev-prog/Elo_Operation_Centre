...
export default function RefundStatusQueuePage() {
  ...
  const steps = [
    { label: 'Step 1', count: data.filter(r => String(r.step) === '1').length, color: '#4d9fff' },
    { label: 'Step 2', count: data.filter(r => String(r.step) === '2').length, color: '#8b5cf6' },
    { label: 'Step 3', count: data.filter(r => String(r.step) === '3').length, color: '#ffb547' },
    { label: 'Complete', count: complete.length, color: '#00d4a0' },
  ]
  ...
  return (
    <div className="flex flex-col flex-1">
      <Topbar
        ...
        subtitle="Tracks refund requests and return cases"
        onRefresh={load}
      ...
      <div className="p-6 flex flex-col gap-5">
        {error && (
          <div className="bg-[#0d1117] border border-[#ff4d6a] rounded-xl p-4 text-[#ff4d6a] text-sm">
            ⚠️ {error}
          </div>
        )}
        ...
        <div className="bg-[#0d1117] border border-[#1e2a35] rounded-xl p-5">
          ...
          <div className="flex gap-3">
            {steps.map((s) => (
              <div key={s.label} className="flex-1 rounded-lg p-3 text-center" style={{ background: `${s.color}10`, border: `1px solid ${s.color}25` }}>
                <div className="font-mono text-xl font-medium mb-1" style={{ color: s.color }}>{s.count}</div>
        ...
        <div>
          ...
          <div className="bg-[#0d1117] border border-[#1e2a35] rounded-xl overflow-hidden">
            <div className="flex items-center justify-between px-4 py-3 border-b border-[#1e2a35]">
              ...
              <Pill color={active.length > 0 ? 'amber' : 'green'}>{active.length} active</Pill>
            </div>
                  ...
                  {displayed.length === 0 ? (
                    ...
                    [...displayed].reverse().map((r, i) => (
                      <tr key={i} className="border-b border-[#1e2a35]/50 hover:bg-[rgba(255,255,255,0.01)] transition-colors">
                        ...
                        <td className="px-4 py-2.5 text-[11px] text-[#a0aec0] max-w-[180px]">
                          <span className="block truncate" title={r.reason || '—'}>{r.reason || '—'}</span>
                        </td>
                        <td className="px-4 py-2.5 text-[11px] text-[#718096] max-w-[160px]">
                          <span className="block truncate" title={r.condition || '—'}>{r.condition || '—'}</span>
                        </td>
                        <td className="px-4 py-2.5 font-mono text-[11px] text-[#8b5cf6]">{r.step || '—'}</td>
                        <td className="px-4 py-2.5"><StatusBadge value={r.status || '—'} /></td>
                        <td className="px-4 py-2.5 font-mono text-[10px] text-[#718096] whitespace-nowrap">
                          {r.completedAt ? fmtTs(r.completedAt) : '—'}
                        </td>
                      </tr>
                    ))
                  )}
