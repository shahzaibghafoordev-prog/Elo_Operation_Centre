...
import type { OrderStatusQueue } from '@/types'

export default function OrderQueuePage() {
  const [data, setData] = useState<OrderStatusQueue[]>([])
  ...
  const load = useCallback(async () => {
    setRefreshing(true)
    try {
      const res = await fetch('/api/sheets/order-queue')
  ...
  return (
    <div className="flex flex-col flex-1">
      <Topbar title="Order Status Queue" subtitle="Customer order status queries in progress"
      ...
      <div className="p-6 flex flex-col gap-5">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="bg-[#0d1117] border border-[#1e2a35] rounded-xl p-4">
            <div className="text-[9px] font-bold tracking-[0.1em] uppercase text-[#4a5568] mb-1.5">Active in Queue</div>
            <div className="font-mono text-2xl font-medium text-[#8b5cf6]">{active.length}</div>
          </div>
          <div className="bg-[#0d1117] border border-[#1e2a35] rounded-xl p-4">
            <div className="text-[9px] font-bold tracking-[0.1em] uppercase text-[#4a5568] mb-1.5">Total Queries</div>
            <div className="font-mono text-2xl font-medium text-[#a0aec0]">{data.length}</div>
          </div>
        </div>

        <div>
          <SectionHeader title="Order Status Queue" />
          <div className="bg-[#0d1117] border border-[#1e2a35] rounded-xl overflow-hidden">
            <div className="overflow-x-auto max-h-[600px] overflow-y-auto">
              <table className="w-full border-collapse">
                <thead>
                  <tr className="bg-[rgba(0,0,0,0.2)]">
                    {['Timestamp','Phone','Order #','Status','Notes'].map(h => (
                ...
                <tbody>
                  {data.length === 0
                    ? <EmptyState icon="🛍️" title="Order Status Queue is empty" subtitle="Queries appear as customers ask about their orders" />
                    : [...data].reverse().map((r, i) => (
                      <tr key={i} className="border-b border-[#1e2a35]/50 hover:bg-[rgba(255,255,255,0.01)]">
                        ...
                        <td className="px-4 py-2.5 text-[11px] text-[#718096] max-w-[250px] truncate">{r.agentNote || '—'}</td>
                      </tr>
                    ))
                  }
                </tbody>
