...
export default function MessagesPage() {
  ...
  const load = useCallback(async () => {
    setRefreshing(true)
    try {
      const res = await fetch('/api/sheets/messages')
  ...
  const filtered = search
    ? messages.filter(m => m.msg.toLowerCase().includes(search.toLowerCase()) || m.cat.toLowerCase().includes(search.toLowerCase()))
    : messages
          ...
          <div className="bg-[#0d1117] border border-[#1e2a35] rounded-xl overflow-hidden">
            <div className="flex items-center justify-between px-4 py-3 border-b border-[#1e2a35]">
              <span className="text-[11px] font-bold uppercase tracking-wide text-[#718096]">
                {messages.length} messages across {data?.tgTabCount || 0} customers
              </span>
            ...
            <div className="overflow-x-auto max-h-[500px] overflow-y-auto">
              <table className="w-full border-collapse">
                ...
                <tbody>
                  {filtered.length === 0 ? (
                    <tr>
                      <td colSpan={6} className="px-4 py-16 text-center text-[#4a5568] text-sm">
                        {messages.length === 0 ? '💬 No messages yet — Alia will populate this as customers interact' : 'No results for your search'}
                      </td>
                    </tr>
                  ) : (
                    [...filtered].reverse().slice(0, 200).map((r, i) => (
                      <tr key={i} className="border-b border-[#1e2a35]/50 hover:bg-[rgba(255,255,255,0.01)]">
                        ...
                        <td className="px-4 py-2.5 text-[11px] text-[#a0aec0] max-w-[220px]">
                          <span className="block truncate" title={r.msg}>{r.msg || '—'}</span>
                        </td>
