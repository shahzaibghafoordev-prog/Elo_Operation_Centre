...
import { calcStats, buildActivityFeed, buildVolumeData } from '@/lib/utils'
...
export async function GET() {
  try {
    ...
    const activity = buildActivityFeed(data, 25)
    ...
    const categoryData = cats.map(c => ({
      name: c,
      count: data.messages.filter(m => m.cat.toLowerCase() === c.toLowerCase()).length
    }))
    ...
    const sentimentData = sents.map(s => ({
      name: s,
      count: weekMsgs.filter(m => m.sentiment.toLowerCase() === s.toLowerCase()).length
    }))

    return NextResponse.json({ stats, activity, volume, categoryData, sentimentData, tgTabCount: data.tgTabCount })
  } catch (err: unknown) {
